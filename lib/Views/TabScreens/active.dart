...
class ActiveTabs extends StatefulWidget {
  const ActiveTabs({super.key});

  @override
  _ActiveTabsState createState() => _ActiveTabsState();
}

class _ActiveTabsState extends State<ActiveTabs> {
  ...
  Future<List<Map<String, dynamic>>> _getPendingOrders() async {
    ...
    for (var doc in usersSnapshot.docs) {
      ...
      for (var orderDoc in orderSnapshot.docs) {
        ...
        // Check for case-insensitive 'approved' status
        var status = orderData?['status']?.toString().toLowerCase() ?? '';
        if (status == 'process') {
